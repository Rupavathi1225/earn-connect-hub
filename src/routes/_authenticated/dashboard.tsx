...
export const Route = createFileRoute("/_authenticated/dashboard")({
  ...
  head: () => ({ meta: [{ title: "Dashboard — Global Prime" }, { name: "description", content: "Your rewards dashboard." }] }),
  component: Dashboard,
});
              ...
              <div className="p-5 text-sm text-gray-700">
                {subTab === "admin" && (
                  <>
                    ...
                    <p>Thanks for being a part of Global Prime Community. Kindly pay attention to mentioned points as they are important.</p>
                    <ol className="list-decimal ml-5 mt-3 space-y-1 text-xs">
                      ...
                      <li>Do not use VPN or proxy when using Global Prime. Doing so will get you banned permanently.</li>
                    </ol>
                  </>
                )}
                {subTab === "news" && (
                ...
                {subTab === "verify" && (
                  <>
                    ...
                    {profile.verified ? (
                      ...
                      <>
                        <p>We have detected that your account is not verified yet. Account verification is necessary to use Global Prime as it helps us in providing our users quality surveys by filtering our cheaters/fake users.</p>
  ...
  return (
    <div className="bg-[#1a1c3a] text-white rounded-lg p-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {ready.map((o) => (
          <button key={o.id} type="button" onClick={() => openWall(o, user.id)} className="bg-white text-[#1a1c3a] rounded-lg py-5 px-2 text-center font-bold text-sm hover:shadow-md transition flex flex-col items-center gap-2">
            <WallLogo name={o.display_name} logoUrl={o.logo_url} urlTemplate={o.url_template} provider={o.provider} />
            <span>{o.display_name}</span>
          </button>
        ))}
        {ready.length === 0 && <div className="col-span-full text-xs text-white/50 text-center py-4">No offerwalls configured.</div>}
      </div>
    </div>
  );
}
