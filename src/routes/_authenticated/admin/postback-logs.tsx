...
function PL() {
  ...
  useEffect(() => { supabase.from("postback_logs").select("*").order("created_at", { ascending: false }).limit(100).then(({ data }) => setItems(data ?? [])); }, []);
  return (
    <div>
      <h1 className="text-lg font-bold text-[#1a1c3a] mb-3">📡 Postback Logs</h1>
      <div className="bg-white rounded-lg overflow-hidden">
        <table className="w-full text-xs">
          <thead className="bg-[#1a1c3a] text-white"><tr>{["Time", "Provider", "TxID", "User", "Points", "Valid", "Processed", "Error"].map((h) => <th key={h} className="p-2 text-left">{h}</th>)}</tr></thead>
          <tbody>
            {items.map((l) => (
